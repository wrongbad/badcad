...
class Solid:
    ...
    def orient(self, direction, origin=(0, 0, 0), x_hint=None):
        ...
        `x_hint` sets the roll about the axis: local +x points as close to
        it as it can (default: world z, or world x if direction is near z)."""
        z = np.array(direction, dtype=float)
        z /= np.linalg.norm(z)
        if x_hint is None:
            x_hint = (1, 0, 0) if abs(z[2]) > 0.9 else (0, 0, 1)
        x = np.array(x_hint, dtype=float)
        x = x - (x @ z) * z
        if np.linalg.norm(x) < 1e-9:
            raise ValueError('x_hint is parallel to direction')
        x /= np.linalg.norm(x)
        y = np.cross(z, x)
